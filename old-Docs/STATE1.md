...
`old-Docs/PLAN1.md` § Milestone gates: *"An arbitrary PNG becomes a mask, runs in physical units, and
...
M5's port; it is deliberately untouched here (`old-Docs/TASKS1.md` § T010 Notes — Phase 0's job
...
- Created `old-Docs/PLAN1.md` (11 tasks, dependency graph, session map, milestone gates, risks).
- Created `old-Docs/TASKS1.md` (full contract per task: goal, depends-on, I/O, acceptance criteria,
  constraints that bite, notes).
- Created `old-Docs/STATE1.md` (this file) and `PROMPTS/templates/session-prompt-template.md`.
...
- Paste `PROMPTS/003-t003-moving-lid-cavity.md` into a fresh session. It runs `/start-task T003`.
  Q-003 (lid corner cells) should be decided and logged there. `old-Docs/PLAN1.md` § Risks flags T003 as
...
**Not done / deferred**
- Nothing from the T003 contract. Zou–He was not implemented — the valve in `old-Docs/PLAN1.md` § Risks
...
- Worth carrying to T007: a `D = 21` cylinder in a `121`-row channel is **17.6% blockage** and
  ...
  `>= 8 D` downstream. This is the check doing the job `old-Docs/PLAN1.md` § Risks assigned it, before
...
- The contract's `forces` validation (uniform flow, no obstacle, `|Cd| < 1e-6`) is trivially true —
  ...
  answer. Criterion strengthened, not relaxed; the row in `old-Docs/TASKS1.md` says so.
...
- Paste `PROMPTS/007-t007-render-live-sink-cylinder.md` into a fresh session. It runs
  ...
  T007 is **M3** and the first rung since session 3; `old-Docs/PLAN1.md` § Risks flags "cylinder shows no
...
- Paste `PROMPTS/010-t010-performance-pass.md` into a fresh session. It runs `/start-task T010`.
  ...
  `old-Docs/STATE1.md` § Performance baseline is still empty and T010 fills it. Known starting points:
  ...
  the step loop. T010 re-runs **all four** rungs, and `old-Docs/PLAN1.md` § Risks says revert rather
