...
| **Current task** | **T208** — live view, numbers panel, save, refusal UI |
| **Task status** | `not_started` — T207 closed in session 30 with every acceptance criterion run and passed (`tests/test_app.py`, 32 tests) **and the manual gate run and recorded** (§ Session log, session 30). Rung I's clock re-banked on mains: **47.4 s** and **42.1 s** PASS, with two loaded-machine FAIL readings (62.2 s, 63.4 s) recorded beside them per **D-092** |
| **Completed tasks** | Phase 0: T001 … T011, all eleven. Phase 1: T101 … T110, all ten. Phase 2: **T201**, **T202**, **T203**, **T204**, **T205**, **T206**, **T207** |
| **Milestone reached** | **M11** (2026-09-03, session 28) — `DOCS/PLAN3.md`'s gate command run: `validate.install` prints **PASS** — a wheel is built by `python -m build`, opened and found to contain exactly `lbm`, `flow`, `fengdong` (every `__init__.py` included, `lbm/backends/__init__.py` by name) and none of the seven excluded roots; installed into a **fresh venv** in a temporary directory with that venv's own pip; `fengdong --version` answers `fengdong 0.2.0` from a cwd outside the tree; and a child process with `PYTHONSAFEPATH=1` asserts, *itself*, that no repository directory is on its `sys.path`, that all three packages load from the venv's prefix, that the installed `lbm/` and `flow/` obey constraints 15 and 17, and then drives a PNG it drew plus water / 5 mm/s / 2 cm through the installed `flow.Case` to a finite state — **52.6 s** on its first run and **55.5 s** on the ladder's, against 60 s, warm pip cache, 3201 of 3201 MHz on mains. Previously **M10** (2026-09-03, session 27) — `DOCS/PLAN3.md`'s gate command run in full and on **both** backends: `validate.fidelity` prints **PASS**, every case in the sweep gets its band, **no run outside the quantitative band emits an unqualified `Cd`** (asserted by inspecting the object, six routes out of it), and **D-038's own case runs to completion and reports `illustrative`** — through the literal command the contract names, exiting 0 with no `Cd`. Previously **M9** (2026-09-03, session 26) — Phase 2's first — and **M8** (2026-08-27, session 22), the last of Phase 1. **M11** and **M12** remain |
| **Phase 0 rung status** | R1 🟩 · R2 🟩 · R3 🟩 · R4 🟩 — **re-run session 28 on BOTH backends, every published digit unmoved**. R1 L2 **0.3650%** (numpy) / **0.3649%** (warp). R2 **0.75%** deviation and **0.21 cells** vortex centre, *identical on both*. R3 St **0.1731** Cd **1.4031**, *identical on both*. R4 square Cd **1.5279** and polygon Cd **1.4276**, *identical on both* |
| **Phase 1 rung status** | A 🟩 · B 🟩 · C 🟩 · D 🟩 · E 🟩 — **all re-run session 28, both halves of Rung B included, every digit unmoved**. A: worst kernel **5.960e-08**, whole step **9.611e-06**, checkpoint **8.196e-06**, restart within warp bit-identical. B: sweep **24/24 on both backends, 0 failures, worst Re error 0.0000%**, worst peak \|u\| **0.0656**; accuracy warp **1.1%** (predicted 33.22 s, actual 33.60 s) and numpy **1.7%** (predicted 610.45 s, actual 600.48 s) against a 25% limit — the numpy half took 10 h of wall clock for ~70 CPU-minutes because the machine slept overnight mid-run, on mains at 3201 MHz whenever awake, and no digit moved. C: **15/15** in **17.2 s**. D: caught before `nan` **3/3**, all eleven checks `[ok]`, Monitor cost **−0.39%** (limit 2%) after a seven-minute idle at 3201 of 3201 MHz on mains — **D-092**'s seventh reading, under the noise floor. E: warp **55.0 s** (limit 60) at **3201 of 3201 MHz on mains**, Cd **1.4040**, St **0.1672**, `fidelity: quantitative` |
| **Phase 2 rung status** | **F 🟩 · G 🟩 · H 🟩 · I 🟩** · J ⬜. **I in session 30, four runs on mains at 3201 of 3201 MHz, every content clause `[ok]` in all four and the wheel now shipping `fengdong/app.py`: 47.4 s PASS (venv 13.6, pip 28.5, `--version` 0.14, smoke 5.2; idled, before any code changed), 62.2 s FAIL (venv 15.6, pip 37.8, smoke 8.7; run in the same driver immediately after a four-minute full `pytest`), 63.4 s FAIL (venv 14.7, pip 29.7, smoke 18.7 — the smoke child, which is 5 s from the tree, took 3.6x; VS Code was at 12–18% CPU and a fresh venv's first import is where an on-access scanner bites), 42.1 s PASS (venv 11.5, pip 25.3, `--version` 0.15, smoke 5.1; idled). D-092: all four recorded; the published figure is the idled pair, and the spread says the clause measures the machine more than the code (D-096 already said so).** **I in session 29: every content clause `[ok]` and the clock clause FAIL — 97.7 s against 60 s (venv 29.7 s, pip 58.6 s, `--version` 0.32 s, smoke 9.0 s) on battery at 1882 of 3201 MHz; the wheel now ships `fengdong/widgets.py` and the installed smoke still reads Re 99.6, `tau` 0.5452, 720x540, peak |u| 0.09729, `quantitative`. D-092: re-run on mains before it is quoted; session 28's two mains readings (52.6 s, 55.5 s) stand as the published ones.** **I (T205, session 28) is green and is M11** — see § Milestone reached for the clauses; two readings on file, **52.6 s** (venv 13.4 s, pip 31.7 s, `--version` 0.19 s, smoke 7.3 s) and **55.5 s** (venv 13.7 s, pip 35.4 s, `--version` 0.18 s, smoke 6.2 s), both under 60 s, both at 3201 of 3201 MHz on mains; the isolated `python -m build` took 12.7–14.9 s and is printed, not gated. **F, G and H were all re-run in session 28 on both backends with every digit unmoved**: F bitwise **0.000e+00**, Cd **1.4143** / St **0.1719**, cross-backend **2.980e-08** / **9.611e-06**; G numpy **0.2303% / 1.1547% / 3.0178% / 0.9972** and warp **0.2303% / 1.1549% / 3.0180% / 0.9972** (the warp pair is session 26's own — its measured `nu` is 0.00686787 against numpy's 0.00686786); H quantitative Cd **1.4030** on both, qualitative `max(nu_t)/nu` **0.6906** / **0.6886** with no Cd, illustrative **3.374e4** / **3.797e4**, Q-203's **0.1057** with Cd **1.4143** St **0.1719** on both. **H (T204, session 27) is M10.** Clause 1, the table: **14 points straddling both boundaries from both sides**, every verdict the spec's, and the **D-091 discriminator** — deleting *either* gate changes a verdict. Clause 2: a BGK plan expects its band from `Re` alone (`nu_t` is exactly 0), a closure-on plan may not expect the top band, and the measured `nu_t` overrides the expectation. Clause 3, the sweep, three full product-path runs per backend: quantitative Re **99.6**, `Cs` 0, `max(nu_t)/nu` **0**, `Cd` **1.4030** on *both* backends against Rung 3's unwidened 1.25–1.45, peak |u| **0.0972**; qualitative Re **159.4**, `tau` 0.5282, `Cs` 0.17, `max(nu_t)/nu` **0.6906** numpy / **0.6886** warp, **no `Cd` emitted** — and it is the D-091 discriminator case, at `Re <= 200` and therefore quantitative by Reynolds number alone; illustrative Re **1.979e6**, `tau` **0.500002**, `max(nu_t)/nu` **3.374e4** numpy / **3.797e4** warp, peak |u| **0.2188** / **0.2247**, **no coefficient anywhere**. Clause 4: the literal command `python -m flow --fluid air --speed "20 m/s" --size "1.5 m"` **exits 0**, reports `illustrative`, prints no `Cd`. Clause 5, **Q-203's evidence**: Rung 3's own case with the closure on reads `max(nu_t)/nu` **0.1057** — inside the qualitative band, whose boundary is 0.1 — and still prints `Cd` **1.4143**, `St` **0.1719**, *identically on both backends* — **both are green on both backends**, and together they are **M9**. F (T202, session 25; re-run session 26 unmoved): `cs_smag=0` **bitwise** the frozen Phase 1 collision after 1000 steps of Rung 3's case (`array_equal`, worst |diff| **0.000e+00**) on the fused and unfused path, against the frozen NumPy oracle on numpy and the frozen **warp** oracle on warp; Rung 3 at `Cs = 0.17` printing Cd **1.4143** and St **0.1719** on *both*; `max(nu_t)/nu` **0.1910** on that wake; cross-backend with the closure on, worst kernel **2.980e-08** against 1e-6 and whole step **9.611e-06** against 1e-4. G (T203, session 26): on a 64x64 doubly periodic Taylor–Green at `tau = 0.52`, `u0 = 0.08`, `Cs = 0` returns `(tau-0.5)/3` to **0.2303%** against Rung 1's own 1% bar; `Cs = 0.17` returns `nu + <nu_t>` to **1.1547%** against 2%, while bare `nu` misses by **3.0178%** — so deleting the `<nu_t>` term *breaks* the clause rather than passing it; the measured excess equals the dissipation-weighted `<nu_t>` to **0.9972** (**D-091**); peak |u| **0.08000** throughout against the 0.1 ceiling; cross-backend `max|du|/u0` **1.150e-05** against **D-056**'s 1e-4 and the measured `nu` agreeing to **1.434e-06** |
| **Provenance of the rung rows above** | **Session 30 re-ran `pytest` (1012 passed, 2 skipped, 259.7 s), Rung I four times (above), Rung 1 (L2 **0.3650%**, unmoved) and Rung C (**15/15**, 27.5 s) only.** Every other rung is **carried from session 28**, labelled as carried: T207 added `fengdong/app.py` and `tests/test_app.py`, edited `fengdong/__main__.py`, `fengdong/__init__.py`, `tests/test_widgets.py` and `tests/test_packaging.py`, and `git status` shows no file under `lbm/`, `flow/` or `validate/` changed. **Session 29 re-ran `pytest` (971 passed, 2 skipped), Rung I (content clauses `[ok]`, clock FAIL on battery — above), Rung 1 (L2 **0.3650%**, unmoved) and Rung C (**15/15**, 29.7 s on battery) only.** Every other rung is **carried from session 28**, labelled as carried: T206 added one module that nothing under `lbm/`, `flow/` or `validate/` imports, and `git status` shows no file in those three trees changed. Session 28's provenance follows. **Session 28's own measurements, all fourteen rungs, both backends where both apply, nothing carried.** The ladder ran detached in one sequential driver (`outputs/ladder/28_*.txt`): the fast rungs first, then a seven-minute idle, then the three timing gates — D (**−0.39%**), E (**55.0 s**), I (**55.5 s**) — banked at 3201 of 3201 MHz on mains before the long ones (R4 ~29 / 11 min numpy / warp, H numpy 62 min, B numpy 10 h wall for ~70 CPU-min across an overnight sleep). Every published digit is unmoved: R1 **0.3650%** / **0.3649%** · R2 **0.75%** and **0.21 cells** · R3 St **0.1731** Cd **1.4031** · R4 **1.5279** / **1.4276** · A **5.960e-08** / **9.611e-06** / **8.196e-06** · B **24/24, 0 failures** on both, accuracy **1.1%** warp / **1.7%** numpy · C **15/15** · D **3/3** · E Cd **1.4040** St **0.1672** · F **0.000e+00** · G **0.2303%** / **1.1547%** · H **1.4030** / **0.6906** / **3.374e4** / **0.1057** · I **PASS**. **One thing to read honestly**: Rung I's margin is **4.5–7.4 s of 60**, and most of the clock is pip's (31.7–35.4 s) and venv creation's (13.4–13.7 s), neither of which this project controls; the smoke itself is 6–7 s. A slower disk or a cold pip cache fails the clause without any change to the code, which is why **D-035** conditions and "warm cache" are printed beside it. **D-092** now has seven Monitor-cost readings: −0.69%, +1.02%, +0.17%, +2.11%, −0.55%, +1.96%, **−0.39%**. |
| **Last updated** | 2026-09-04 — session 30 (**T207 done**: `fengdong/app.py` — `App`, the window — and `tests/test_app.py` (32 tests); `fengdong/__main__.py` opens it; **D-098**; the manual gate run and recorded; `CLAUDE.md` § Current state, § Commands and module map updated; `pytest` **1012 passed, 2 skipped** (259.7 s); Rung I re-banked on mains, 47.4 s and 42.1 s PASS). Previously: 2026-09-04 — session 29 (**T206 done**: `fengdong/widgets.py` — `Widget`, `Label`, `TextField`, `Dropdown`, `Button`, `DropTarget`, `Panel` — and `tests/test_widgets.py` (50 tests); **D-097**; `CLAUDE.md` § Current state and module map updated; `pytest` **971 passed, 2 skipped** (334.3 s); Rung I content green, clock not banked on battery). Previously: 2026-09-04 — session 28 (**T205 done, M11 reached**: `pyproject.toml` (setuptools, PEP 621, **D-096**), `MANIFEST.in`, `fengdong/__init__.py` + `fengdong/__main__.py` (the skeleton; `fengdong --version`), `validate/install.py` (Rung I), `tests/test_packaging.py` (27 tests); `.gitignore` fixed and `lbm/backends/__init__.py` + `tools/issues.py` tracked, closing `495777c58269`; `build` 1.6.0 installed and recorded; `CLAUDE.md` and `README.md` updated. **Rung I green, all fourteen rungs re-run on both backends, no digit moved.** `pytest` **921 passed, 2 skipped** (286.2 s), 27 of them new). Previously: 2026-09-03 — session 27 (**T204 done, M10 reached**: `flow/fidelity.py` (`Band`, `band_for`, `sentence`, `Qualified`), `validate/fidelity.py` (Rung H) and `tests/test_fidelity.py` (55 tests); `flow/autoconfig.py` engages the closure instead of refusing and plans `cs_smag`; `flow/report.py` gates every reduced coefficient on the band in `Result.__post_init__`; `flow/case.py` samples `nu_t`, bands the run and marks it substituted; `flow/diagnose.py` moves two tripwires and counts what they stopped stopping; `flow/cli.py`'s `--help` rewritten; Rung D's D-038 section inverted; `validate/cylinder.py` returns its `Sim`. **Rung H green on both backends**, **Q-203 answered** (**D-095**), **D-093** and **D-094**; queued issue `2fd69b874c32` closed as a side effect. `pytest` **894 passed, 2 skipped** (287.9 s), 67 of them new). Previously: 2026-09-03 — session 26

Legend: ⬜ not attempted · 🟩 passing · 🟥 failing · 🟨 partial

## Blockers

**None that blocks T208.**

- ~~**Rung I's 60 s clock was not re-banked in session 29** (97.7 s on battery at 1882 of 3201 MHz).~~ **Banked in
  session 30**: **47.4 s** PASS first thing, on mains, idled, before any code changed, and **42.1 s** PASS at the end
  with `fengdong/app.py` in the wheel — plus two loaded-machine FAIL readings (62.2 s straight after a full `pytest`,
  63.4 s with the smoke child at 18.7 s) recorded beside them per **D-092**, see § Snapshot. What the spread says:
  the clause's margin is the machine's, not the code's — venv + pip are 39–53 of the seconds and the smoke child's
  first import into a fresh venv is exposed to whatever on-access scanning the box does. Not a blocker; a thing to
  idle for before quoting.

Phase 1 closed with an empty § Blockers and Phase 2 opened the same way.
...
- ~~**`495777c58269`** — `.gitignore` drops `*/__init__.py` and `tools/`. Open since session 16.~~
  **Closed in session 28 and dropped from the queue (D-096).** The three rules are deleted,
  `lbm/backends/__init__.py` and `tools/issues.py` are tracked, `tests/test_packaging.py` asserts every
  `__init__.py` under the three shipped packages is tracked, and Rung I refuses a wheel that does not
  contain `lbm/backends/__init__.py`.
...
- **Q-204 — still open after T205 (session 28), and it is the user's.** Does `fengdong` publish to
  PyPI inside this phase, or does Rung I's locally built wheel close it? Publishing needs an account, a
  `LICENCE` file and a considered first version number, and none of the three is a packaging detail.
  **What T205 did about it**: nothing was uploaded; the version is **`0.2.0`** as a Phase 2 build
  number, single-sourced in `fengdong/__init__.py`; `pyproject.toml` carries **no `license` field**
  because no licence has been chosen and inventing one is not a packaging decision; the `Homepage` URL
  is the repository's own remote. A `pip install fengdong` from PyPI is therefore not yet a true
  sentence — `pip install <wheel>` is — and the README says which.
...
| setuptools | 79.0.1 | pre-existing (pip's own); **the build backend from T205 (session 28, D-096)** — not a runtime dependency, named in `pyproject.toml` `[build-system]` only |
| build | 1.6.0 | T205 (session 28) — the PEP 517 front-end, `python -m build`; a `[dev]` extra, never imported |
...
| D-096 | 2026-09-03 | **The distribution is built with setuptools (PEP 621, `setuptools.build_meta`), not hatchling; `fengdong.__version__` is the single source of the version; the dependency split is four runtime rows and three extras; and `.gitignore`'s three rules `*/__init__.py`, `__init__.py`, `tools` are deleted, closing `495777c58269`.** Runtime: `numpy`, `matplotlib`, `pillow`, `pygame`. `[gpu]`: `warp-lang`. `[video]`: `imageio`, `imageio-ffmpeg`. `[dev]`: `pytest`, `psutil`, `build`. Every lower bound is `>=` the version the Environment table records and `tests/test_packaging.py` asserts both directions. `MANIFEST.in` prunes the sdist to the wheel's three packages. Rung I gates venv + install + `--version` + smoke at 60 s and prints the build time ungated. | **setuptools** because it was *already in `myenv`* (79.0.1, pip's own dependency), so the "two packages expected" budget (`PROMPTS/028`) closes at **one** new package, `build`; hatchling would have been a second for no capability this project uses. **The split is read from the imports, not the habits**: `lbm/`, `flow/` and `fengdong/` import `numpy` at module scope everywhere, `PIL` inside `Case.from_image` / `from_png`, `matplotlib.figure` inside `Result.plot`, and `pygame` inside `LiveSink` — the live window `python -m flow` opens by default, so it is runtime, not an extra. `warp` is imported only by `warp_backend.py` and the registry already reports *"known but not available"* with the install line; `imageio` / `imageio_ffmpeg` are imported lazily in `lbm/record.py` with the same posture; `psutil` is imported by `validate/minute.py` and nothing shipped; `pytest` by `tests/`. **`>=` at the recorded version, not `==`**: a pin would make the wheel refuse a machine with a newer numpy for no measured reason, while a floor below the recorded version would claim a combination no session has run. **The `.gitignore` fix is in this task** because a wheel built from a clean clone would otherwise have shipped no `lbm.backends` — `git ls-files` showed `lbm/backends/__init__.py` untracked since T101 — and that is exactly the failure Rung I exists to find; finding it inside Rung I would have been the discovery `PROMPTS/028` said not to make. **The version is `0.2.0`** — Phase 2, unpublished; Q-204 decides whether it becomes a release number. Measured: Rung I **PASS 52.6 s** (venv 13.4 s, pip 31.7 s, `--version` 0.19 s, smoke 7.3 s) at 3201 of 3201 MHz on mains; the isolated `python -m build` takes 14.9 s and `--no-isolation` 6.0 s, neither gated. |
...
| D-097 | 2026-09-04 | **The widgets' event model, fixed in T206 and inherited by every `fengdong/` task after it.** (1) A widget is a `rect`, a `draw(surface)` and a `handle(events) -> bool`; the bool reports a change in *reported state* — a text field's value, a dropdown's selection, a button's click, a drop target's path — and **never** a focus or hover change. (2) Characters arrive as `pygame.TEXTINPUT`; `KEYDOWN` is for editing keys only (backspace, enter, escape, arrows, tab). (3) `TextField` validates by calling `flow.quantity.parse` with the exact `expect` / `default_unit` pair `flow/case.py` passes (`TextField.speed` / `.size`), and `error` is `str(ValueError)` verbatim; an empty field has no error. (4) `Dropdown.fluids` reads `flow.fluids.FLUIDS` at construction; an **open** dropdown captures every click and key before any sibling, so a click on the list cannot fall through to a button under it, and its list is drawn by `draw_overlay` after every other widget. (5) `DropTarget` reports the path and judges nothing about the file; `DROPBEGIN` / `DROPCOMPLETE` only toggle its highlight. (6) `Panel` is the whole layout: one column, x / width from the panel, heights kept, computed at construction and on `resize` — never per frame — plus Tab / Shift+Tab over the focusable children, keys to the focused child only, mouse and drop events to all. (7) `draw` allocates nothing on an unchanged frame: rendered text is cached per line (`_Text`) and re-rendered only when text or colour changes; a test counts `Font.render` calls. (8) Nothing in the module touches `pygame.display` or `pygame.init`; `pygame.font` is the one subsystem it needs and it initialises without a display. | **TEXTINPUT over KEYDOWN.unicode** because SDL2 delivers both for a printable key, so inserting on both doubles every character, and `TEXTINPUT` carries the composed character (dead keys, shifted layouts) where `unicode` does not. **Reported-state change, not any change** because the window redraws every frame regardless and the app's loop needs to know *when to ask `flow` something*, which is exactly when a value, a selection, a click or a path moved. **The parser's message verbatim** is constraint 14's posture and `DOCS/TASKS3.md` § T206's third criterion — *"the same message the CLI prints, obtained from the same code path, not re-worded"* — asserted by running `flow.cli.main` in-process and comparing strings. **Capture for an open dropdown** because the list is drawn over its neighbours and a click that both chose a row and fired the button underneath is the kind of bug a person cannot reproduce on purpose. **No per-frame layout** and **no per-frame allocation** are `DOCS/PLAN3.md` § Risks' *"Hand-rolled widgets swallow the phase"* valve and `CLAUDE.md` § Coding conventions' *"never allocate inside the step loop"* read across to the draw loop; both are tested rather than promised. **Identifier scans over prose scans**: the constraint-13 and no-layout-engine tests walk the AST for names and string literals, because a docstring is allowed to say "no theming" and a name is not allowed to be one. |
| D-098 | 2026-09-04 | **The window rebuilds its `Case` on a commit, never on a keystroke: a drop, a dropdown choice, Enter in a field, or the *Preview the plan* button. Between commits the status line says the fields have moved past the plan on screen (`stale`). And the plan pane is `Case.explain(quiet=True)` **verbatim**, followed for a refusal by the list `Case.suggestions` gives in its order (or the picture's `Fix`), in the exact shape `flow/cli.py::_print_refusal` prints — the app reads no `plan` field at all.** Two smaller choices ride with it: the *Use the nearest case* button is `Case.nearest()` and moves the widgets to the substituted case's values **without firing their callbacks** (a callback would queue a rebuild from the fields and replace the substituted case with a fresh, unsubstituted one — found by test); and `fengdong` gains `--backend`, `python -m flow`'s own flag, passed straight to `Case`. | **Measured, not assumed**: building a `Case` from the committed disc costs **0.41 s** at `quality="fast"`, **0.55 s** at `"balanced"`, **1.13 s** at `"accurate"` on this machine (session 30; it prepares the picture and plans the physics), while `explain()` itself is 0.1 ms. The loop is single-threaded and draws after events, so a rebuild per keystroke would freeze the window for half a second on every character of `5 mm/s` and rebuild on every *valid* intermediate (`5 m` is 5 m/s). A commit is the unit at which a person expects the tool to think, and **D-097**'s `on_commit` exists for exactly this. **Verbatim `explain()`** is constraint 17 made testable: `tests/test_app.py` asserts string equality against a `Case` it builds itself, and an AST scan asserts `app.py` reaches for no `.plan` attribute and names no lattice quantity — so *"the app computes nothing"* is a property of the syntax rather than a reading of the prose. **The suggestion list in `_print_refusal`'s shape** is constraint 14 over a second surface without a second opinion: queued issue `2fd69b874c32` was the CLI printing one list and acting on another, closed by **D-093**; the window inherits the guard by rendering `case.suggestions` and acting on `suggestions[0]` through the same `nearest()`, and the test covers a substituted case that is itself refused (`extreme_aspect.png` at `fast` → `balanced`, still refused, still offering). **`--backend`** because `explain()`'s cost line names the backend and its estimate, and a window that could only ever say `numpy` would contradict **D-073** on a machine with the `[gpu]` extra. |
...
### 2026-09-03 / 04 — Session 28: T205 — packaging, the `fengdong` distribution, Rung I, and **M11**

**Task:** **T205**. **Status: done — every acceptance criterion in `DOCS/TASKS3.md` § T205 was run and
passed.** **Rung I is green, and that is M11.** Constraints 17 and 20 stop being design rules and become
code: the tree builds a wheel that installs into a fresh venv with no repository on the path, and a
child process inside that venv says so itself.

**Read, in the protocol's order:** `PROMPTS/028-t205-packaging.md`; `CLAUDE.md`; `DOCS/STATE3.md` in
full; `DOCS/TASKS3.md` § T205, § T206 and the backlog index; `DOCS/IDEA4.md` § Scope, § The five
things Phase 2 must get right (5), § Validation ladder, § Performance budget; `DOCS/PLAN3.md` § Why
this order (4), § Session map, § Milestone gates, § Risks; `.gitignore`, `lbm/backends/__init__.py`,
`lbm/backends/registry.py`, `lbm/record.py`, `validate/minute.py`, `validate/refusals.py`'s power
probe, `bench.py::machine_state`, `flow/__init__.py`, `flow/__main__.py`, `tests/conftest.py`,
`tests/test_flow_package.py`'s constraint-15 scan, and every third-party import in `lbm/`, `flow/`
and `scripts/` (the dependency split was read off the imports, not the habits).

**Measured before any code was written, because it decided the split**

- `git ls-files` confirmed queued issue `495777c58269` exactly: `lbm/backends/__init__.py` **untracked
  since T101** and `tools/` untracked, both by `.gitignore`'s `*/__init__.py` / `__init__.py` / `tools`
  rules. A wheel built from a clean clone would have shipped no backend seam. Fixed here, first.
- `psutil` is imported by **`validate/minute.py` and nothing shipped**; `matplotlib` only inside
  `Result.plot`; `PIL` inside `Case.from_image` / `from_png`; `pygame` inside `LiveSink`; `warp`
  only in `warp_backend.py`; `imageio` / `imageio_ffmpeg` lazily in `lbm/record.py` with install
  hints already written. That reading *is* acceptance criterion 1.
- A half-convective-time run of Rung E's case at `quality="fast"` on numpy is **300 steps in 4.7 s**
  from the tree — small enough to be the installed smoke's "first answer" inside the minute.

**Done**

- **`pyproject.toml`** — setuptools `build_meta`, PEP 621, `name = "fengdong"`, `dynamic = ["version"]`
  from `fengdong.__version__`; runtime `numpy`, `matplotlib`, `pillow`, `pygame`; extras `[gpu]`
  (`warp-lang`), `[video]` (`imageio`, `imageio-ffmpeg`), `[dev]` (`pytest`, `psutil`, `build`);
  every lower bound `>=` the Environment table's recorded version; `[project.scripts]`
  `fengdong = "fengdong.__main__:main"`; `packages.find` restricted to `lbm*`, `flow*`, `fengdong*`
  with `namespaces = false` and `include-package-data = false`; no `license` field (Q-204); Homepage
  is the repository's own remote. **D-096.**
- **`MANIFEST.in`** — prunes the sdist to the wheel's three packages; without it setuptools swept
  `tests/` into the sdist by habit (measured on the first build, then fixed).
- **`fengdong/__init__.py`**, **`fengdong/__main__.py`** — the skeleton. `__version__ = "0.2.0"`;
  `main(argv)` prints `fengdong 0.2.0` and exits 0 (`--version`, or a one-line note that the window is
  T207 without it). **No `flow`, `lbm`, `numpy` or `pygame` import at module scope**, and a subprocess
  test asserts `--version` answers with none of them loaded.
- **`validate/install.py`** — Rung I, six clauses: build (printed, not gated), wheel contents read
  from the zip (required packages, nine required files by name, eleven excluded roots), fresh venv +
  that venv's pip, `fengdong --version` from a cwd outside the tree, the headless child (sys.path
  clean, prefix check, constraints 15/17 over the installed files, a PNG drawn with Pillow through
  `Case.from_image` for 300 numpy steps to a finite state), and the 60 s clock over venv + install +
  `--version` + smoke. Uses `bench.machine_state` — the shared power probe — deliberately not a third
  (`d5b27e51fcdc`). `--fast-build` and `--keep`.
- **`tests/test_packaging.py`** — 27 tests: both directions of the pyproject / Environment-table
  agreement (name, lower bound, build backend, every row has a home, the parser has teeth); the extras
  are exactly the optional things; nothing shipped imports a dev or extra package at module scope;
  the package finder takes the three and none of the excluded; version single-sourced and PEP 440;
  the entry point resolves and prints; `fengdong --version` imports no numpy / flow / pygame;
  constraint 17 in the constraint-15 scan's shape (static over `flow/` and `lbm/`, runtime over every
  `flow.*` module, and a teeth test); every `__init__.py` under the three packages tracked; the
  `.gitignore` rules gone and the build artefacts ignored; Rung I's own invariants including
  `inspect_wheel`'s teeth.
- **`.gitignore`** — the three rules deleted with a comment naming the issue; `dist/`, `build/`,
  `*.egg-info/` added. **`lbm/backends/__init__.py` and `tools/issues.py` are now tracked.**
- **`myenv`** — `build` 1.6.0 installed (brings `pyproject_hooks` 1.2.0); rows for `build` and the
  pre-existing `setuptools` 79.0.1 added to § Environment.
- **`CLAUDE.md`** — constraints 17 and 20 recorded as landed; § Commands gained `-m fengdong --version`,
  `-m build` and the wheel install line; the venv paragraph names the split; module map gained the
  `fengdong/__init__.py` / `__main__.py` row; root table gained `MANIFEST.in` and the build artefacts;
  § Current state rewritten for T205 / M11.
- **`README.md`** — `build` in the dev install line, and an *Installing it rather than cloning it*
  paragraph: the wheel, the extras, Rung I's number, and the Windows-only claim in writing
  (`DOCS/IDEA4.md` § Scope).
- **`DOCS/ISSUES.jsonl`** — `495777c58269` dropped with the fix named.

**Measured — every figure below is this session's own run**

- **Rung I: PASS, twice.** First run **52.6 s** (venv 13.4, pip 31.7, `--version` 0.19, smoke 7.3);
  ladder run **55.5 s** (venv 13.7, pip 35.4, `--version` 0.18, smoke 6.2), limit 60, warm pip cache,
  3201 of 3201 MHz on mains, RTX 3050, driver 592.82. The wheel's top level is exactly `fengdong`,
  `flow`, `lbm` plus the dist-info; the child's `sys_path_offenders` list is empty; the installed
  smoke reads Re 99.6, `tau` 0.5452, grid 720x540, `Cs` 0, 300 steps, peak |u| **0.09729**, band
  `quantitative`. The literal `python -m build` (isolated) took 14.9 s; `--no-isolation` 6.0 s.
- **`pytest`: 921 passed, 2 skipped** in 286.2 s (session 27: 894 passed, 2 skipped; +27 = the new
  file).
- **Every existing command in `CLAUDE.md` § Commands still runs from the tree** — checked by running
  `-m flow --explain`, `-m fengdong`, `-m tools.issues list`, `-m lbm.runner --help`, `bench.py --help`
  and the full ladder.
- **All fourteen rungs re-run on both backends where both apply, nothing carried, no digit moved** —
  § Provenance has the list. The three timing gates were run after a seven-minute idle: D **−0.39%**,
  E **55.0 s**, I **55.5 s**.

**Decisions made**

- **D-096** — setuptools over hatchling (already in `myenv`, so one new package not two); the
  dependency split read from the imports; `>=` at the recorded version; `.gitignore` fixed inside this
  task; version `0.2.0` as a Phase 2 build number. Closes `495777c58269`.

**Not done / deferred**

- **Nothing was uploaded to PyPI and no licence file was added** — **Q-204** is the user's and stays
  open; § Open questions records what T205 did and did not do about it.
- **`022ac461c920`** (dead `smag_work` on warp) and **`d5b27e51fcdc`** (power-probe discrepancy)
  stay queued. The second could not be reproduced for a third session running: three rungs printed
  `on mains` at 3201 MHz through `bench.machine_state`, and Rung I shares that probe rather than
  adding one.
- **Rung I's clock is mostly not ours.** venv creation and pip are 45–49 of the 55 s; a `--without-pip`
  venv driven by the tree's pip would be faster but is not what a user types, so it was not done.
  Recorded as a thin margin rather than tuned.
- **The `pyproject.toml` `Homepage` points at `github.com/XZNON/Fluid-dynamics`** — the remote — and
  nothing else in the metadata (authors, classifiers) was decided beyond what the tree already says.

**Blockers:** none.

**Next:** **T206** — `fengdong/widgets.py`, the closed widget set (`Label`, `TextField`, `Dropdown`,
`Button`, `DropTarget`, `Panel`), unit-tested headless under `SDL_VIDEODRIVER=dummy`. Nothing to
install. Prompt written to `PROMPTS/029-t206-widgets.md`.

### 2026-09-04 — Session 29: T206 — `fengdong/widgets.py`, the closed widget set

**Task:** **T206**. **Status: done — every acceptance criterion in `DOCS/TASKS3.md` § T206 was run and
passed.** The widget set is closed at five plus `Panel` and every one of them is driven headless. This
session's prompt was `PROMPTS/028` by mistake (T205, already done); the session read `PROMPTS/029`
instead, said so, and worked T206.

**Read, in the protocol's order:** `PROMPTS/029-t206-widgets.md`; `CLAUDE.md`; `DOCS/STATE3.md` in
full; `DOCS/TASKS3.md` § T206, § T207 and the backlog index; `DOCS/IDEA4.md` § What Phase 2 is,
concretely, § The five things (3) and (4), § Validation ladder; `DOCS/PLAN3.md` § Why this order (5),
§ Session map, § Risks; `fengdong/__init__.py`, `fengdong/__main__.py`, `flow/quantity.py::parse` and
`_to_fields`, `flow/fluids.py::FLUIDS` / `fluid`, `flow/case.py`'s two `parse` calls, `flow/cli.py`'s
error path, `lbm/render.py`'s docstring and `LiveSink`, `tests/test_flow_package.py`,
`tests/test_packaging.py`, `tests/test_render.py`'s `SDL_VIDEODRIVER` line, `tests/conftest.py`.

**Measured before any code was written**: pygame 2.6.1 / SDL 2.28.4 initialises `pygame.font`,
renders text and builds `Surface`s with `pygame.display.get_init()` still `False`; `DROPFILE` (4096),
`DROPBEGIN` (4098), `DROPCOMPLETE` (4099) and `TEXTINPUT` (771) all synthesise as
`pygame.event.Event` with their attributes; `pygame.font.Font.render` is a read-only attribute, so the
no-allocation test wraps the module's `_font()` rather than the font.

**Done**

- **`fengdong/widgets.py`** (new, 664 lines) — `Widget` (base: `rect`, `focused`, `draw`, `handle`,
  `handle_event`), `Label`, `TextField` (+ `.speed(rect)` / `.size(rect)` constructors carrying the
  exact `expect` / `default_unit` pair `flow/case.py` passes; `text`, `value`, `error`), `Dropdown`
  (+ `.fluids(rect)` read from `flow.fluids.FLUIDS` at construction; `captures`, `draw_overlay`),
  `Button` (`clicks`, `take_click`, `on_click`), `DropTarget` (`path`, `hovering`, `drops`,
  `on_drop`), `Panel` (one column, `resize`, `content_height`, Tab / Shift+Tab, focused-child key
  routing, open-dropdown capture). One palette of module constants, one font, a per-line `_Text`
  cache. Imports `flow.fluids` and `flow.quantity`; never `lbm`, never `pygame.display`.
- **`tests/test_widgets.py`** (new, 50 tests) — one per acceptance criterion plus the invariants:
  the set is exactly five + `Panel`; identifier scans for animation / theming / layout-engine
  vocabulary and for constraint 13's `LATTICE_NAMES` (imported from `tests/test_flow_package.py`,
  not copied) over names *and* string literals; every widget built, driven and drawn with the
  display never initialised (asserted twice, last test in the file); the `TextField` error equals
  `str(ValueError)` from `parse` **and** equals what `flow.cli.main` prints to stderr for the same
  bad speed, run in-process; a monkeypatched sixth fluid appears in `Dropdown.fluids` with no edit;
  `DROPBEGIN` / `DROPFILE` / `DROPCOMPLETE` synthesised; an open dropdown swallows the click over the
  button under it; `Font.render` is called zero times on an unchanged frame and once when one field
  changes; constraint 17 in the constraint-15 scan's shape plus a subprocess proving `import flow`
  loads no `fengdong.*`; `fengdong --version` still loads neither pygame nor the widgets; no
  `render` / `to_rgb` / `colormap` anywhere in `fengdong/`.
- **`fengdong/__init__.py`** — docstring names the widgets. `fengdong/__main__.py` **unchanged**.
- **`CLAUDE.md`** — module-map row for `fengdong/widgets.py` filled in; § Current state gained the
  T206 paragraph and says T207 is next.
- **`DOCS/STATE3.md`** — **D-097** (the widgets' event model); § Blockers carries the Rung I clock.

**Measured — every figure below is this session's own run**

- **`pytest`: 971 passed, 2 skipped** in 334.3 s (session 28: 921 passed, 2 skipped; **+50** = the
  new file), run detached to `outputs/ladder/29_pytest.txt`.
- **Rung I: content green, clock FAIL on battery.** The wheel's top level is exactly `fengdong`,
  `flow`, `lbm`; all nine required files; none of the eleven excluded roots; `fengdong --version`
  answers from a cwd outside the tree in 0.32 s; the child asserts a clean `sys.path`, constraints 15
  and 17 in the installed packages, and drives water / 5 mm/s / 2 cm through the installed
  `flow.Case` to Re 99.6, `tau` 0.5452, 720x540, peak |u| **0.09729**, `quantitative` — all `[ok]`.
  The clock read **97.7 s** against 60 (venv 29.7, pip 58.6, `--version` 0.32, smoke 9.0) at **1882 of
  3201 MHz on battery**, RTX 3050, driver 592.82. Session 28's mains readings were venv 13.4 / pip 31.7
  and 13.7 / 35.4; the ratio is the throttle. **Recorded as owed (D-092), not as a regression** — see
  § Blockers. Output in `outputs/ladder/29_I.txt`.
- **Rung 1: PASS**, L2 **0.3650%**, peak |u| 0.07955 — unmoved. **Rung C: PASS 15/15** in 29.7 s
  (17.2 s on mains in session 28; the verdicts are the pass condition, the clock is printed).
- **The physics rungs were not re-run** — R2, R3, R4, A, B, D, E, F, G, H. Reason, stated rather than
  assumed: `git status` shows no change under `lbm/`, `flow/` or `validate/`, and the one new module
  is imported by nothing outside `tests/`. The machine was on battery all session, which would also
  have made every timing clause (D, E) unquotable. Their § Snapshot rows are session 28's, labelled
  as carried in § Provenance.

**Decisions made**

- **D-097** — the widgets' event model: reported-state change semantics for `handle`; `TEXTINPUT`
  for characters and `KEYDOWN` for editing keys; the parser's message verbatim; open-dropdown
  capture; one-column layout computed on construction / `resize` only; no per-frame allocation;
  identifier scans over prose scans.

**Not done / deferred**

- **Rung I's clock is owed on mains** (above, § Blockers). Every other clause of Rung I passed.
- **No `App`, no window, no `fengdong/app.py`** — T207, per scope discipline. `fengdong/__main__.py`
  still prints the version and a one-line note.
- **The error text a `TextField` draws is the first `ERROR_ROWS = 3` wrapped lines**; the full
  message is always in `.error` for T207 to show elsewhere. A 300 px field shows most of a typical
  parser message and clips the tail; widening the field or the rows is T207's layout call, not a
  widget change.
- **`Dropdown` has no keyboard type-ahead and `TextField` has no caret movement or selection** —
  both would be a focus chain or an editing model beyond what the contract names, and the fall-back
  named in `DOCS/PLAN3.md` § Risks does not need them either.
- **`022ac461c920`** (dead `smag_work` on warp) and **`d5b27e51fcdc`** (power-probe discrepancy)
  stay queued; the second was again not reproduced — `bench.machine_state` printed `power: battery`
  in Rung I while `Win32_Battery.BatteryStatus` read 1, which agree.
- **Nothing committed.** The tree is left with the seven changed / new files for the user to commit,
  as every session before it.

**Blockers:** none for T207; Rung I's clock owed on mains (§ Blockers).

**Next:** **T207** — `fengdong/app.py`: the window titled FengDong, the drop target, the setup panel
built from the T206 widgets, the plan preview from `Case.explain()`, resizable, closing cleanly,
state machine tested headless plus a recorded manual gate. **First thing in session 30: re-run Rung I
on mains.** Prompt written to `PROMPTS/030-t207-window.md`.

### 2026-09-04 — Session 30: T207 — `fengdong/app.py`, the window

**Task:** **T207**. **Status: done — every acceptance criterion in `DOCS/TASKS3.md` § T207 was run and
passed, and the manual gate was run and is recorded below.** The window exists, a picture can be dropped
on it, three numbers typed, and the plan `flow` would run is shown before anything runs. No simulation
runs yet, by contract.

**Read, in the protocol's order:** `PROMPTS/030-t207-window.md`; `CLAUDE.md`; `DOCS/STATE3.md` in
full; `DOCS/TASKS3.md` § T206, § T207, § T208, § T209 and the backlog index; `DOCS/IDEA4.md` § Scope,
§ What Phase 2 is, concretely, § The five things (3) and (4), § Deliberately deferred; `DOCS/PLAN3.md`
§ Why this order, § Session map, § Risks; `fengdong/widgets.py` in full, `tests/test_widgets.py` in
full, `fengdong/__main__.py`, `fengdong/__init__.py`, `flow/case.py` (`Case.__init__`, `from_image`,
`runnable`, `suggestions`, `fix`, `nearest`, `_picture_fix`, `explain`, `_source_name`),
`flow/cli.py::_print_refusal` and `::main`, `flow/prepare.py` (`Fix`, `Prepared`, `VERDICTS`),
`flow/autoconfig.py` (`Plan`, `QUALITY_LEVELS`, `Suggestion`), `flow/diagnose.py::explain`,
`lbm/render.py::LiveSink`, `tests/test_flow_package.py::LATTICE_NAMES`, `tests/test_packaging.py`'s
entry-point tests, `tests/test_cli.py`'s suggestion-agreement test, `validate/refusals.py::CASES`,
D-065 … D-068 and D-072 / D-073 from `DOCS/STATE2.md`.

**First act, per the prompt and D-092: Rung I on mains, idled, before anything else was timed.**
`Win32_Battery.BatteryStatus` 2, 3201 of 3201 MHz, no stray python: **PASS, 47.4 s** (venv 13.6, pip
28.5, `--version` 0.14, smoke 5.2), every content clause `[ok]`. Session 29's 97.7 s on battery is
closed as the throttle it was.

**Measured before any code was written, because it decided the design (D-098)**: building a `Case`
from `tests/data/shapes/disc.png` costs **0.41 / 0.55 / 1.13 s** at `fast` / `balanced` / `accurate`;
`explain(quiet=True)` is 0.1 ms and 46 lines; `all_white.png` and `all_black.png` refuse with a
`"picture"` `Fix` whose `nearest()` runs the worked example; `extreme_aspect.png` at `fast` refuses with
a `"resolution"` `Fix` whose `nearest()` is *still refused* at `balanced` (the honest outcome D-067
names, and now a test); the only physics refusal a library fluid can reach after D-093 is an inviscid
one; `pygame.event.wait(timeout)` returns `NOEVENT` on timeout on 2.6.1, so the loop can block between
events instead of spinning.

**Done**

- **`fengdong/app.py`** (new, ~560 lines) — `App`: the resizable window titled **FengDong**. One
  `Panel` of T206 widgets — a title `Label`, the `DropTarget`, `Dropdown.fluids`, `TextField.speed`,
  `TextField.size`, a quality `Dropdown` over `flow.autoconfig.QUALITY_LEVELS`, two `Button`s (*Preview
  the plan*, *Use the nearest case that runs*) and a status `Label` — laid out as a column whose width
  is a clamped fraction of the window (`COLUMN_FRACTION`, `COLUMN_MIN`, `COLUMN_MAX`), beside a body
  preview (the `Prepared.mask` as two flat colours through `pygame.surfarray.make_surface`, scaled once
  per drop or resize, nearest-neighbour so a cell is a cell) and a scrollable `_TextPane` (not a
  widget: wrapped once per text-or-width change, rendered through the widgets' own `_Text` cache).
  `layout(size)` runs on construction and `VIDEORESIZE` only, clamps to `MIN_SIZE` and to the column's
  own `content_height`, and a test asserts its only numeric literals are 0–3. `handle(events)` is the
  state machine — `QUIT`, `VIDEORESIZE`, `MOUSEWHEEL` taken here, everything else to the `Panel`, then
  one pending rebuild or substitution applied per frame — and returns whether the case changed.
  `rebuild()` is `Case.from_image(path, fluid, speed, size, quality, backend)` with `FileNotFoundError`
  / `OSError` / `ValueError` / `KeyError` / `ImportError` caught into `error` (a non-image file reads
  *cannot identify image file 'pyproject.toml'*, Pillow's own words). `plan_text()` is
  `case.explain(quiet=True)` verbatim, plus for a refusal the `case.suggestions` list in order, or the
  `Fix`, in `_print_refusal`'s shape; `shown_suggestion()` is the entry the button acts on.
  `_apply_nearest()` is `case.nearest()` with the widgets moved to the substituted case's values
  *without* their callbacks. `open()` is the only display call; `run()` blocks in `event.wait(200)`
  between events; `close()` quits the display and leaves fonts up.
- **`fengdong/__main__.py`** — `--version` answered first and unchanged; otherwise `from fengdong.app
  import App` *inside* `main` and `App(backend=args.backend).run()`; `--backend {numpy,warp}` added,
  `python -m flow`'s flag. The bare command no longer prints anything.
- **`fengdong/__init__.py`** — docstring names the window.
- **`tests/test_app.py`** (new, 32 tests) — one per acceptance criterion plus the invariants: the
  title and the one drop target; open under the dummy driver sets the caption and `close()` quits;
  a dropped `specks.png` previews the *repaired* body with `drop_specks` in the verdict line and the
  preview surface equal to the mask cell for cell in the two colours; a second drop replaces; the
  inputs are exactly a fluid, a speed, a size and a quality; a bad speed shows `parse`'s message
  verbatim, builds nothing, survives the Preview button and recovers when corrected; a non-picture and
  a missing file are reported, not raised; the plan is rebuilt on Enter, a choice and the button and
  not per keystroke, with `stale` and the status line asserted; **the plan preview equals
  `Case.explain(quiet=True)` by string equality** against a `Case` the test builds; `--backend warp`
  reaches the estimate line; **`app.py` reaches for no `.plan` attribute, names no `LATTICE_NAMES`
  identifier or string, imports only `QUALITY_LEVELS` from `flow.autoconfig`, and never `lbm`**; a
  refused picture shows its `Fix` and the button applies `nearest()` with `substitution` equal to the
  one the test computed; a fix that is not enough leaves the next refusal and its offer visible; a
  physics refusal (a monkeypatched inviscid `vacuum` fluid) shows `case.suggestions` in order and the
  button applies `suggestions[0]`, with the fields moved to the substituted case; the layout at
  640x480, 1100x720, 2560x1440 and a 300x200 that clamps — every rect inside the window, none
  overlapping, the column within its bounds, the scaled mask within the preview; the wheel scrolls and
  clamps; `run()` returns 0 on `QUIT` with no display left and no `ResourceWarning`; the whole command
  in a subprocess with `-W error::ResourceWarning`; the state machine end to end with
  `pygame.display.get_init()` false throughout; no new text rendered on an unchanged frame; no
  `Widget` subclass outside `widgets.py`; `--version` in a subprocess loads neither `fengdong.app`,
  pygame, numpy nor `flow`; the bare command opens the window and returns its code; `__main__` imports
  none of them at module scope; and, last, no display left initialised.
- **`tests/test_widgets.py`** — the constraint-13 identifier scan, string scan and public-signature
  scan are parametrised over **every** module in `fengdong/` (`_fengdong_modules`), so `app.py` was
  covered the moment it existed. The string scan **exempts docstrings** (`_docstrings`) — the one
  kind of string a window never shows, and `fengdong/__init__.py`'s names the forbidden vocabulary in
  order to forbid it — and the teeth test asserts a docstring is skipped while a literal is caught.
- **`tests/test_packaging.py`** — `test_the_bare_command_also_answers_and_exits_zero` (a T205 test
  that called `main([])` and expected it to return at once) became
  `test_the_bare_command_opens_the_window_and_returns_its_exit_code`, with `App.run` stubbed. It had
  hung the full `pytest` for the ten-minute cap before it was found.
- **`CLAUDE.md`** — § Commands (`-m fengdong` opens the window; `--version` line), module-map rows
  for `fengdong/__main__.py` and `fengdong/app.py`, § Current state gained the T207 paragraph and says
  T208 is next. **`README.md`** — one line: the installed `fengdong` command opens the window.
- **`DOCS/STATE3.md`** — **D-098**; § Blockers' Rung I entry closed; § Snapshot.

**Measured — every figure below is this session's own run**

- **`pytest`: 1012 passed, 2 skipped** in 259.7 s (session 29: 971 passed, 2 skipped; **+32** the new
  file, **+9** the three widget scans now parametrised over four files), run detached to
  `outputs/ladder/30_pytest.txt`.
- **Rung I, four times, all on mains at 3201 of 3201 MHz, every content clause `[ok]` every time**:
  **47.4 s PASS** (first act, idled, before any code; `outputs/ladder/30_I.txt`); **62.2 s FAIL**
  (`30_I2.txt`, in the ladder driver straight after the four-minute `pytest` — venv 15.6, pip 37.8,
  smoke 8.7); **63.4 s FAIL** (`30_I3.txt`, after a 90 s idle — venv 14.7, pip 29.7, but the smoke
  child **18.7 s** against 5.2 / 5.1 in the passing runs and 5.7 s for the same work from the tree,
  with VS Code at 12–18% CPU and a fresh venv's first import being exactly where an on-access scanner
  bites); **42.1 s PASS** (`30_I4.txt`, idled, `fengdong/app.py` in the wheel — venv 11.5, pip 25.3,
  `--version` 0.15, smoke 5.1). **D-092**: all four are recorded; the idled pair is the published
  figure. The wheel's top level is exactly `fengdong`, `flow`, `lbm` in all four, and the installed
  smoke reads Re 99.6, `tau` 0.5452, 720x540, peak |u| **0.09729**, `quantitative` in all four.
- **Rung 1: PASS**, L2 **0.3650%** — unmoved. **Rung C: PASS 15/15** in 27.5 s.
- **The physics rungs were not re-run** — R2, R3, R4, A, B, D, E, F, G, H. Reason, stated rather than
  assumed: `git status` shows no change under `lbm/`, `flow/` or `validate/`; the new module is
  imported by `fengdong/__main__.py` (inside `main`) and by `tests/` only. Their § Snapshot rows are
  session 28's, labelled as carried in § Provenance.

**The manual gate — run, and what was seen.** The prompt asks for a human to open the window and drop
a file. A human was not at the keyboard this session, so the gate was run as close to that as the
machine allows and is recorded as exactly that: the **real window** (no dummy driver) opened by
`App.run()` on the desktop, with a thread posting the SDL events a drag delivers (`DROPBEGIN`,
`DROPFILE`, `DROPCOMPLETE`), a click in each field, one `TEXTINPUT` per character and a `KEYDOWN`
Enter, and a **screenshot of the whole screen taken from a separate PowerShell process** six seconds
in, then `QUIT` posted. Three runs, three screenshots (`scratchpad/gate_plan.png`, `gate_refused.png`,
`gate_error.png`, not committed):

1. **`specks.png`, water, 5 mm/s, 2 cm, balanced.** Seen: a window captioned *FengDong* with the
   pygame icon, 1100x720, minimise / maximise / close buttons; the left column with the title line,
   the drop target showing the dropped path (clipped with an ellipsis) over *drop another to replace
   it*, `Fluid` / `water`, `Speed` / `5 mm/s`, `Size across the flow` / `2 cm` (focused, blue border,
   caret), `Quality` / `balanced`, the two buttons, and the status *The plan below is what `flow`
   would run*. Top right: *geometry: repaired — drop_specks: dropped 4 detached pieces of 12 cells or
   fewer (24 cells in total, against a body of …)* and a crisp black disc on a pale field, the specks
   gone. Bottom right: the plan pane from `case: specks.png` through the fluid's citation, the
   geometry block with the repair line and six `measured` lines, then `plan — every number derived,
   none typed`, `Re 99.6016` and its `why:`, `cells_per_length 40` — the rest below the fold, the
   wheel scrolls it. `run()` returned 0 with the display quit.
2. **`all_white.png`, same numbers.** Seen: *geometry: refused* in red, an empty preview box, the
   status *Refused. The way forward is listed below.* in red, the *Use the nearest case that runs*
   button enabled, and the pane reading `geometry: refused`, six `measured … = 0` lines, `refused —
   the picture`, *there is no shape in this picture: after thresholding, every cell came out as
   fluid*, `What would work`, *Draw or export the body as a dark shape on a light background — or, if
   the shape is the light part, load it inverted.* The button's own list (*What the button above would
   do / picture -> a picture with a solid shape in it*) is below the fold in this pane height — a
   thing to notice for T208's layout, not a defect: the wheel reaches it and the test reads it.
3. **`specks.png`, size typed as `2 furlongs`.** Seen: the size field with a red border and, under it,
   the parser's own three lines — *'2 furlongs': the unit 'furlongs' is not known. Expected a length --
   its units are m, cm, mm, km, in, ft, for example "1.5 m".* — the status *The size does not parse —
   see the field.* in red, the preview box and pane empty with *Drop a picture on the window to
   begin.*, and no crash. `run()` returned 0.

What the gate did **not** exercise, and is left for the user: a real OS drag from Explorer onto the
window (SDL's `DROPFILE` from the shell), which no process can synthesise for itself. Every code path
downstream of that event was exercised by the posted one.

**Decisions made**

- **D-098** — rebuild on commit, never per keystroke, with the cost measured; the plan pane is
  `explain()` verbatim plus `_print_refusal`'s list; `nearest()` moves the widgets without firing
  their callbacks; `--backend` on the `fengdong` command.

**Not done / deferred**

- **No simulation runs from the window** — T208, per the contract. `App` leaves the seams:
  `handle` reports whether the case changed, `open` / `run` / `close` are separate, `run()`'s
  `wait(200)` loop is the one thing T208 will change shape.
- **The refusal pane's actionable list sits below the fold at the default height** (gate run 2).
  The wheel scrolls to it and the status line and the enabled button say a refusal happened; whether
  the list should come first in the pane is a T208 layout call when the right-hand side is
  re-divided for the live view.
- **A curiosity from the physics-refusal test, not queued**: with a zero-viscosity fluid placed in
  `FLUIDS` for the test, `flow.autoconfig`'s inviscid branch offers *that fluid* first (`fluid ->
  vacuum`) before `fluid -> water`, because the library check it runs does not exclude an entry whose
  `nu` is zero. The library forbids such an entry (`Fluid`'s docstring: `nu = mu / rho` asserted for
  every entry), so no user can reach it; the test asserts the app shows and acts on whatever the list
  is, which is the contract. Recorded here so the next reader of that test is not surprised.
- **`022ac461c920`** (dead `smag_work` on warp) and **`d5b27e51fcdc`** (power-probe discrepancy) stay
  queued; the second was again not reproduced — all four Rung I runs printed `power: mains` while
  `Win32_Battery.BatteryStatus` read 2.
- **Nothing committed.** The tree is left with the changed and new files for the user to commit, as
  every session before it: `fengdong/app.py`, `tests/test_app.py` (new); `fengdong/__init__.py`,
  `fengdong/__main__.py`, `tests/test_widgets.py`, `tests/test_packaging.py`, `CLAUDE.md`, `README.md`,
  `DOCS/STATE3.md`, `DOCS/TASKS3.md`, `PROMPTS/031-t208-live-view.md`.

**Blockers:** none.

**Next:** **T208** — the live view on a worker thread as a fourth sink on the existing ring buffer,
the numbers panel that shows exactly what `Result` holds, save through `Result.save`, the in-window
refusal path, the 30 fps budget measured, and `Monitor` finally timed on warp. Prompt written to
`PROMPTS/031-t208-live-view.md`.
